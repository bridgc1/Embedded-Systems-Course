...
void SysTick_Wait10ms(uint32_t delay){
  uint32_t i;
  for(i=0; i<delay; i++){
		// 26.67 MHz x (1s/1000ms) x 10 ms = 266700 for 10 ms
    SysTick_Wait(266700);  // wait 10ms (assumes 26.67 MHz clock)  T_cycle = 1/26.67 MHz = 37.5 ns
  }
}

// Time delay 10ns using busy wait.
// This assumes 120 MHz system clock.
void SysTick_Wait10us(uint32_t delay){ //delay parameter is actually the number of cycles NOT time itself
  uint32_t i;
  for(i=0; i<delay; i++){
		// 26.67 MHz x (1s/1000000us) x 10 us = 266.7 ~ 267 for 10 ms
    SysTick_Wait(267);  // change this to wait 10us (assumes 120 MHz clock)
  }
}
