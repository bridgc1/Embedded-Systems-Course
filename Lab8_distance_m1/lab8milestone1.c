/* The VL53L1X is run with default firmware settings.
							...
							Updated by: Christina Bridges
							- modification of studio code to complete lab 8
*/
#include <stdint.h>
#include "PLL.h"
#include "SysTick.h"
...
int main(void) {
  ...
  uint8_t dataReady;
	...
	status = VL53L1_RdByte(dev, 0x010F, &modelID); //0x010F identification register
	status = VL53L1_RdByte(dev, 0x0110, &moduleType);
	status = VL53L1_RdWord(dev, 0x010F, &wordData);

	sprintf(printf_buffer,"Model ID: %x\r\n",modelID); // can use %d for decimal formatting
